...
class VideoDecoderTest < Minitest::Test
  ...
  def test_get_frames_played_at
    decoder = TorchCodec::Decoders::VideoDecoder.new("test/support/nasa_13013.mp4")
    frames = decoder.get_frames_played_at(Torch.tensor([0, 1], dtype: :float64))
    assert_equal [2, 3, 270, 480], frames[:data].shape
    assert_equal [2], frames[:pts_seconds].shape
    assert_equal [2], frames[:duration_seconds].shape
  end

  def test_get_frames_played_in_range
    decoder = TorchCodec::Decoders::VideoDecoder.new("test/support/nasa_13013.mp4")
    frames = decoder.get_frames_played_in_range(0, 1)
    assert_equal [30, 3, 270, 480], frames[:data].shape
    assert_equal [30], frames[:pts_seconds].shape
    assert_equal [30], frames[:duration_seconds].shape
  end
end
